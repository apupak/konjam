# Handoff Prompt: Alternate Voice Exploration for Native Tamil Audio (V4)

## 🎯 Context
The Konjam project has a fully functional **Native Audio V3** batch using `kn-IN-Wavenet-A` (female). All 374 audio assets are verified and working. We now want to explore a **friendlier, more natural male voice** for a potential V4 batch.
...
*   **Private Repo**: `git@github.com:apupak/konjam-private.git`
*   **Public Repo**: `https://github.com/apupak/konjam.git`
*   **Current Audio**:
...
*   **Key Scripts**:
    ...
    *   `scripts/Tamil_mapping.py` — 374+ phonetic-to-Unicode mappings.
*   **API Key**: Stored in `.env` (git-ignored), loaded by the generator.

## 🔬 Research Summary: Google Cloud TTS Voice Options for Tamil
...
**Tamil (kn-IN) IS confirmed supported** by Chirp 3: HD.
...
### Existing Wavenet / Neural2 Voices for Tamil
...
> **⚠️ SSML Compatibility**: Chirp 3: HD has **limited SSML support** compared to Wavenet. It supports `<speak>`, `<break>`, `<p>`, `<s>`, `<say-as>`, and `<sub>`, but **may NOT support `<lang>`**. If `<lang>` is not supported, you may need to send plain Tamil text without the SSML wrapper. Test this carefully.

If Chirp 3: HD doesn't work well for Tamil pronunciation, fall back to:
...
3. Keep the existing `Tamil_MAPPING` from `Tamil_mapping.py` — the Unicode input is voice-agnostic.
...
1. [generate_audio.py](file:///Users/apupak/Documents/Konjam-private/scripts/generate_audio.py) — Current generator logic.
2. [Tamil_mapping.py](file:///Users/apupak/Documents/Konjam-private/scripts/Tamil_mapping.py) — All 374 Unicode mappings (reuse as-is).
3. [audio.js](file:///Users/apupak/Documents/Konjam-private/docs/assets/js/audio.js) — Frontend playback logic.
4. [pronunciation_tts_retrospective.md](file:///Users/apupak/Documents/Konjam-private/research/pronunciation_tts_retrospective.md) — How V3 was built.
5. [GEMINI.md](file:///Users/apupak/Documents/Konjam-private/GEMINI.md) — Repo rules (never push scripts/research to public).
...
*   **Private Repo Only**: All changes stay in `Konjam-private`. No public deployment.
*   **Reuse Mappings**: The `Tamil_MAPPING` dictionary is voice-agnostic. Don't regenerate it.

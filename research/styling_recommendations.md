# Konjam Styling Recommendations

This document outlines suggested styling improvements for the Konjam site to align with its Chennai-centric, "Speak first, read never" philosophy.

## 🎨 Color Palette: "Namma Chennai"

Instead of the default `indigo`, we should use colors that evoke the spirit of Chennai.
...
*   **Cubbon Green (`#2E7D32`)**: Secondary color. Inspired by Chennai's lush parks. Used for success states, secondary buttons, and navigational elements.
...
*   **Primary Font**: Use `Inter` or `Outfit` for a modern, tech-friendly feel that resonates with the Chennai IT crowd.
...
> **Prompt**: A minimalist, modern logo for a language learning app called Konjam. The logo should feature a stylized, vibrant orange speech bubble merged with a minimal silhouette of a Chennai street auto-rickshaw. Flat design, solid background (white or transparent), bold lines. Professional, friendly, and high-contrast.

### 2. Hero Section Illustration
> **Prompt**: A high-quality vector illustration of a modern IT professional (gender-neutral) comfortably speaking with a Chennai street vendor. In the background, subtle hints of Cubbon Park greenery and a Metro pillar. Bright, "Gulmohar Red" and "Cubbon Green" color palette. Clean lines, modern "SaaS" illustration style (flat with subtle gradients). No text.
...
> 4. Travel/Transport (A Chennai auto-rickshaw)
...
3.  **Chennai "Swag" Stickers**: Stylized circle stickers with catchy phrases like `"Oota Aitha?"` or `"Sir, Konjam Adjust Maadi"` featuring the new branding. This builds community and organic awareness.
...
### Recommendation
1.  **Phase 1 (Current)**: Apply the "Namma Chennai" branding to the existing MkDocs site. This gives you an immediate visual win.
...
3.  **Chennai "Easter Eggs"**: Add subtle local references in the examples (e.g., mentioning "Corner House" or "MTR") to build cultural familiarity.

# Retrospective: Alternate Voice Exploration for Native Tamil Audio (V4)

## Objective
To explore a friendly, more natural male voice (Chirp 3 HD) for a potential V4 batch of native Tamil audio. We generated the batch and implemented a local toggle to compare the new voice with the existing Native Female (Wavenet) voice.
...
1. **Script Modifications (`scripts/generate_audio.py`)**:
   ...
   - Refined the handling of Tamil text to ensure SSML tags (`<lang xml:lang='kn-IN'>`) are correctly applied and passed to the Google Cloud TTS API.
   - Addressed an issue where `strip_accents()` stripped essential Tamil vowel markers, allowing better fallback pronunciation for unmapped Unicode words.
...
- **Prefix Issue**: Initially, the script utilized an English prompt prefix to assist the Chirp 3 model ("The following word is in Tamil: "). This resulted in the prefix being spoken before every word. It was removed once testing confirmed the Chirp 3 model properly handled the `<lang xml:lang='kn-IN'>` SSML tag.
- **Accent Stripping Issue**: The fallback mechanism originally used `strip_accents()`, which inadvertently corrupted Tamil text by removing necessary vowel signs (e.g., converting `ಬೇಗ ಹೋಗಿ` to `ಬೕಗ ಹೂೕಗ`). The `strip_accents` logic was removed for native words to preserve correct pronunciation.

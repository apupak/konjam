# Retrospective: High-Fidelity Tamil TTS & Pronunciation Logic

## 🎯 Goal
To provide authentic, native-grade Tamil pronunciation for the Konjam project, moving beyond generic Text-to-Speech (TTS) defaults that often struggle with transliterated South Indian languages.

## 🏗️ Technical Evolution

### Phase 1: Engine Upgrade (gTTS to Google Cloud TTS)
- **Problem**: The initial `gTTS` library sounded robotic and lacked nuanced prosody for Indian English/Tamil phonetics.
...
- **Solution**: **Direct Unicode Mapping**.
  - Created `scripts/Tamil_mapping.py` containing 374+ manual mappings of phonetic IDs to actual Tamil script (e.g., `OH-luh-gey` -> `ಒಳಗೆ`).
  - By feeding the engine Tamil script, the `kn-IN` voice automatically applies authentic Tamil prosody, pitch, and duration.
...
1. **Update Mappings**: Add any new phonetic terms to `scripts/Tamil_mapping.py` first.
...
- **Native Mode**: 100% authentic Tamil pronunciations, serving as a "gold standard" for listeners.

...
function updateUI() {
    ...
    else respectRank.innerText = "Thala";
...
function triggerGlobalReset() {
    ...
    resetBtn.innerText = "Konjam adjust pannunga (Request Reset)";
    resetBtn.onclick = () => {
        state.respect = 25;
        state.patience = 50;
        updateUI();
...
function renderLevelFailed(message) {
    ...
    scene.innerHTML = `
        <div class="scene-intro animate-fade-in">
            <h1>Level Failed</h1>
            <p class="scenario-text">${message || "The streets of Chennai can be tough!"}</p>
